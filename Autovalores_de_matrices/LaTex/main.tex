...
\begin{lstlisting}[language=Matlab]
  x_all (: , i ) = x ;
  r = tanh ( x ) ;
  xlp = xlp + (( - xlp + x /1 e -2) / taux ) * dt ;
  input = input + ( - zeta * input + randn * input1 (: , i ) +
  randn * input2 (: , i ) ) * dt ;
\end{lstlisting}
...
La figura \ref{comparacion lineal-no lineal} muestra una comparación entre los resultados obtenidos mediante el modelo de \cite{susman2019} (\textbf{Dcha}) y el mío (\textbf{Izda}) donde se observa cómo no es necesara la no linealidad para mostrar la generación de memorias uardadas como un par de autovalores complejos conjugados. Ambas simulaciones se han llevado a cabo en Matlab usando el script de github de \cite{susman2019} cambiando los parámetros para añadir la linealidad.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\linewidth]{Figures/linearized_seed.png}
  \includegraphics[width=0.45\linewidth]{Figures/nonlinear_seed.png}
...
Sin embargo al no ser capaz de replicar los resultados en python en mi propio código e intentar traducir el código de matlab directamente a python me di cuenta de que si simplemente cambiaba la semilla del generador de números aleatorios el resultado cambiaba drásticamente

\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\linewidth]{Figures/nonlinear_12.png}
  \includegraphics[width=0.45\linewidth]{Figures/nonlinear_123.png}
  \includegraphics[width=0.45\linewidth]{Figures/nonlinear_234.png}
  \includegraphics[width=0.45\linewidth]{Figures/nonlinear_537.png}
  \caption{Diferentes simulaciones para el modelo no lineal en Matlab empleando diferentes semillaspara el generador denúmeros aleatorios: 12, 123, 234 y 537 de izquierda a derecha y de arriba a abajo.}
\end{figure}
